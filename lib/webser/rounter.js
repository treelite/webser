...
Rounter.prototype.match = function (url, method) {
    ...
    if (action && action.hasMethod(method)) {
        res = function (req, rep) {
            ...
            action = action.create(req, rep, action.root, action.path, context);
 ...
 * @param {Object} config
 */
exports.create = function (root, src, context, config) {
    var actionsMap = {};
    var config = config || {};
    var paths = walker.find(src, (config.suffix || 'Action') +'.js$');
